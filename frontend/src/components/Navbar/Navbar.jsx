...
const Navbar = () => {
    ...
    // Line 23: Function to close mobile nav on link click
    const closeMobileNav = () => {
        setMobileNav(false);
    };

    return (
        <nav className='px-4 md:px-8 lg:px-12 py-4 relative'>
            <div className='flex items-center justify-between'>
                ...
                <div className='hidden w-2/6 lg:flex items-center justify-end'>
                    {!isLoggedIn && ( 
                        <> 
                        <Link to="/login" className='px-6 py-3 border-2 border-black rounded-full'>Login</Link>
                ...
                <div className='w-4/6 flex items-center justify-end lg:hidden z-50'>
                    ...
                    >
                        {MobileNav ? <GiCrossMark /> : <IoReorderThreeOutline />}
                    </button>
                </div>
            </div>
                ...
                <div className='h-full flex flex-col items-center justify-center'>
                    {navLinks.map((items, i) => (
                        <Link
                            key={i}
                            to={items.path}
                            className='mb-12 text-4xl hover:font-semibold transition-all duration-300'
                            // Line 74: Close the mobile menu after clicking a link
